...
/*
 * 1. Decrease #ticks > 0 foreach blocked thread
 * 2. If #ticks == 0 then:
 * 2.1 Disable interrupts
 * 2.2 Unblock thread
 * 2.3 Enable interrupts
 */
void resume_thread(struct thread *t, void *aux) {
  if (t->status == THREAD_BLOCKED && t->ticks_blocked > 0) {
    t->ticks_blocked--;

    if (t->ticks_blocked == 0) {
      /* thread_unblock must be called with interrupts off */
      enum intr_level old_level = intr_disable();
      thread_unblock(t);
      intr_set_level(old_level);
    }
